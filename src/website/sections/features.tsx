  ...
  return (
    <div className={` ${container.sectionPadding} ${container.maxWidthGutter}`}>
      <Title className="mb-16">
        ...
        <Heading level="h3" size="5xl" className="max-w-[700px]">
          {DATA.title.heading}
        </Heading>
        <Paragraph size="lg" className="mt-4 max-w-[800px]">
      ...
      <ul className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        {DATA.cards.map((card) => (
          <li key={card.heading} className="p-6 bg-primary-50 rounded-lg">
            ...
            <Heading level="h4" size="xl" className="mt-4">
              {card.heading}
            </Heading>
            <Paragraph size="base" colorScheme="gray-800" className="mt-2">
              {card.paragraph}
            </Paragraph>
          </li>
        ))}
