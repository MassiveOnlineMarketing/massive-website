        ...
        {isMobile ? (
          ...
          <div>
            <button
              ...
              className={`md:hidden fixed right-8 top-8 p-[14px] ${constants.glassFill} shadow-custom-lg rounded-full z-50`}
            >
              <svg
          ...
          // Desktop navigation
          <div
            className={`hidden md:block fixed top-0 w-full z-30 transition-transform duration-500 ease-in-out ${navbarStyle}`}
          >
            <div
              className={`px-6 py-6 mt-4 rounded-2xl  max-w-[1324px] mx-auto ${styles.glass}`}
            >
...
const Drawer = ({ isOpen, setIsOpen }: DrawerProps) => {
  return (
    <div
      className={`fixed top-0 left-0 bottom-0 w-full h-screen bg-white z-50 transform transition-transform duration-200 ease-in-out ${isOpen ? "-translate-x-0" : "translate-x-full"} md:hidden`}
    >
      <button
        aria-label="Close menu"
        onClick={() => setIsOpen(false)}
        className={`fixed right-8 top-8 p-[14px] ${constants.glassFill} shadow-custom-lg rounded-full z-50`}
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          fill="none"
