...
const SwipeNavigation: React.FC<SwipeNavigationProps> = ({ data }) => {
  ...
  return (
    <div
      className={`flex gap-0 md:gap-6 py-2 px-2 bg-[#EFF4FB] my-8 md:w-fit md:px-2 rounded-xl overflow-auto`}
    >
      {data.map((item, index) => (
        <span
          key={index}
        ...
        >
          <p
            className={`whitespace-nowrap rounded-3xl cursor-pointer text-sm leading-5 font-semibold px-6 py-3 ${activeIndex === index ? "text-gradient-primary " : "text-secondary-500"}`}
          >
            {item.heading}
          </p>
        </span>
