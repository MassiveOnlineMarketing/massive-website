...
function CookieConsentBanner() {
  ...
  const savePreferences = () => {
    ...
    for (const segment in consent) {
      setCookie(segment as CookieKeys, consent[segment as ConsentKeys], {
        path: "/",
        expires,
      }); // use the Date object
    }
  ...
  return (
    <div
      className="p-8 m-2 fixed border-primary-500 border-l-4 bg-white shadow-sm bottom-2 z-50 rounded-md w-full max-w-[1300px] left-[49.6%] -translate-x-1/2"
      // style={{ maxWidth: 'calc(100% - 1rem)' }}
      id="cookieConsent-banner"
    >
      <Heading level="h4" size="base" colorScheme="dark">
        {COOKIE_CONSENT_BANNER.heading}
      </Heading>
      <div className="flex flex-col gap-2 mlg:flex-row justify-between w-full bottom-2">
        <Paragraph size="sm" className="max-w-[800px] mr-auto">
          {COOKIE_CONSENT_BANNER.description}
        </Paragraph>
        {/* <p>Je kunt je cookie-instellingen op elk moment wijzigen. Lees ons Cookiebeleid voor meer informatie.</p> */}
        <div
          id="cookieConcent-Buttons"
          className="flex flex-col mlg:flex-row gap-6 pt-4 md:pt-0 items-center"
        >
          <Button
            size="sm"
            variant="primary"
            className="w-full mlg:w-fit"
            onClick={acceptAllCookies}
          >
            {COOKIE_CONSENT_BANNER.accept}
          </Button>
          <Button
            size="sm"
            variant="glass"
            ...
            onClick={acceptAllCookies}
          >
            {COOKIE_CONSENT_BANNER.necessary}
          </Button>
          <CookieSettings
            consent={consent}
          ...
          >
            <TextButton size="sm" variant="text" className="w-full mlg:w-fit">
              {COOKIE_CONSENT_BANNER.settings}
            </TextButton>
          </CookieSettings>
        </div>
      </div>
...
const COOKIE_CONSENT_BANNER = {
  ...
  description:
    "Bij Massive maken we gebruik van cookies om de goede werking en beveiliging van onze websites mogelijk te maken en je de best mogelijke gebruikerservaring te bieden. Door op 'Accepteren' te klikken, stem je in met het gebruik van deze cookies voor advertenties en analyses.",
  accept: "Accepteren",
