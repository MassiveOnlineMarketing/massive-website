...
const AccordionContent: React.FC<AccordionContentProps> = ({ children }) => {
  ...
  useEffect(() => {
    if (contentRef.current) {
      contentRef.current.style.maxHeight = expand
        ? `${contentRef.current.scrollHeight + padding}px`
        : "0";
    }
  }, [expand, padding]);
  ...
  return (
    <div
      ref={contentRef}
      className={cn(
        "max-h-0 overflow-hidden transition-all duration-300 ease-out text-black px-4",
        // `px-${tailwindPadding}  `,
        `${expand ? `pb-4` : ""}`,
      )}
    >
      <div
        className={`absolute -z-20 top-0 -left-5 h-full w-8 rounded-[14px] blur-md bg-gradient-to-t from-[#FFCFA3]/25 from-5.48% via-[#C176FC]/25 via-24.47% via-[#9076FC]/25 via-54.38% to-[#9076FC]/10 to-90.5% transition-opacity duration-500 ease-out op ${expand ? " opacity-100" : "opacity-0"}`}
      ></div>
