        ...
        <div className=" bg-white lg:bg-gradient-to-r from-[#f2f2ff] via-16% via-[#f8f8ff] via-50% via-[#fff] via-84% via-[#f8f8ff] to-[#f2f2ff]">
            ...
            <main className={`pt-[70px] md:pt-[140px] pb-40`}>
                <div className={`${container.maxWidthGutter} relative z-10`}>
                    <section className={`py-12 flex text-center flex-col gap-6`}>
                        ...
                        <div className="mx-auto md:flex md:items-center gap-4 text-gray-500 text-sm font-medium">
                            ...
                            <div aria-label="separator" className="md:block w-[6px] h-[6px] bg-gray-500 rounded-full hidden"></div>
                            <div className="justify-center flex items-center gap-4">
                                <div className="flex items-center gap-2">
                                    <CalendarIcon className='w-5 h-5' />
                                    <time aria-label="published date" dateTime={publishedAt}>{publishedAt}</time>
                                </div>
                    ...
                    <Image className="mx-auto max-w-[800px] rounded-2xl" priority={true}  src={`${PAYLOAD_BACKEND_URL}${blogData.heroImage.url}`} alt={blogData.heroImage.alt} width={blogData.heroImage.width} height={blogData.heroImage.height} />

                    <article className={`mt-[44px] max-w-[800px] mx-auto`}>
                        <RenderBlocks layout={blogData.layout} />
                    </article>

                    <AuthorsCard authors={authors} containerStyles=" pt-[70px] md:mt-[140px]" />
                </div>


                <section className={`text-center relative w-full pt-[70px] md:mt-[140px]`}>
                    <div className={`  ${container.maxWidthGutter}`}>
                        <Heading level='h2' size='5xl' colorScheme='gray-700' className='text-center z-10' >{NIEWUSBRIEF.heading}</Heading>
                        <Paragraph colorScheme='gray-500' className="mt-2 z-10">{NIEWUSBRIEF.paragraph}</Paragraph>
                ...
                <section className={`${container.maxWidthGutter} mt-[70px]`}>
                    {blogData.relatedPosts && <RenderRelatedPosts relatedPosts={blogData.relatedPosts} />}
                </section>
            </main>
...
const NIEWUSBRIEF = {
    ...
    paragraph: 'Blijf op de hoogte binnen de Online Marketing en meer door je in teschrijven in onze nieuwsbrief'
}