...
const RenderRelatedPosts = (posts: any, containerStyles?: string) => {
    ...
    return (
        <>
            <Heading level='h2' size='2xl'>{DATA.heading}</Heading>
            ...
            <div className={cn(
                ...
                {posts.relatedPosts.map((post: BlogPost) => {
                    return <BlogCard key={post.id} {...post} containerStyles="min-w-[300px] xs390:min-w-[320px] md:min-w-[400px] max-w-[450px] snap-start "/>;
                })}
            </div>
        </>
    )
