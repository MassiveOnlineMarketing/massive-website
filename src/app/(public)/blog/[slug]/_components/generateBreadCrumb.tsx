    ...
    return (
        <>
            <nav
                ...
                <Link className='hidden md:block' href={`${BASE_URL}`}>Massive Online Marketing</Link>
                <span className='hidden md:block'> <ChevronRightIcon className='w-4 h-4 mr-1' /> </span>
                <Link href={`${BASE_URL}/blog`}>Blogs</Link>
                <span> <ChevronRightIcon className='w-4 h-4 mr-1' /> </span>
                <Link href={`${BASE_URL}/blog/categorie/${categoryObj.slug}`} className={tagObj?.title ? '' : 'text-gray-500'}>{categoryObj.title}</Link>
                {
                    tagObj?.title &&
                    <>
                        <span> <ChevronRightIcon className='w-4 h-4 mr-1' /> </span>
                        <Link href={`${BASE_URL}/blog/categorie/${categoryObj.slug}`} className='text-gray-500'>{tagObj.title}</Link>
                    </>
                }
            </nav>
