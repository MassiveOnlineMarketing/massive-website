...
const SecondarySidebar = ({
  ...
  const [openGoogleSearchProjectDialog, setOpenGoogleSearchProjectDialog] =
    useState(false);
  ...
  const googleResultAccess =
    useGoogleRefreshToken("search-console") && currentWebsite?.gscUrl
      ? false
      : true;
  ...
  const fetchProjects = async () => {
    ...
    const searchProjects = await getGoogleSearchProjectByWebsiteId(
      currentWebsite.id,
    );

    const newCampaignsChildren = searchProjects.map(
      (project: GoogleSearchProject) => ({
