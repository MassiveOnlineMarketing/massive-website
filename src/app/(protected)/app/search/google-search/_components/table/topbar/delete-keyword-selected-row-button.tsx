...
export const DeleteKeywordSelectedRowButton = ({ selectedRows, onActionFinished }: { selectedRows: any, onActionFinished: () => void }) => {
    ...
    return (
        <DropdownMenu onOpenChange={isOpen => isOpen && handleDeleteClick()}>
            <DropdownMenuTrigger asChild>
                <OutlinedTextButton outlineClassName="outline-red-50 bg-red-50" size='smD' className="ml-1" buttonClassName="px-2">
            ...
            <DropdownMenuContent className="p-2">
                <DropdownMenuGroup className='text-sm leading-5 font-normal'>
                    <DropdownMenuItemEmpty className='flex gap-2'>
                        <button onClick={() => { confirmDelete(); onActionFinished(); }} className=' text-red-500 border border-red-500 px-[12px] py-[6px] rounded-md'>Delete Keywords</button>
                        <button onClick={() => { cancelDelete(); onActionFinished(); }} className=' text-gray-500 px-[12px] py-[6px] rounded-md'>Cancel</button>
                    </DropdownMenuItemEmpty>
                </DropdownMenuGroup>
            </DropdownMenuContent>
        </DropdownMenu>
