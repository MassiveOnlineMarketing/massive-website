    ...
    return (
        <>
            ...
            <div className='py-3 flex justify-between items-center w-full border-b border-dashed border-gray-200'>
                ...
                <Pill color='primary' variant='text'>{keywordData.url?.substring(0, 52)}</Pill>
            </div>
            <div className='py-3 flex items-center w-full border-b border-dashed border-gray-200'>
                <p className='text-gray-800 font-medium'>Position</p>
                {
                    keywordData.position && (
                        <Pill color={getColorAndText(keywordData.position, CONFIG.position).color} variant='icon' className='ml-auto mr-2 flex gap-2 items-center'>
                            <PieChartUserResultDetails item={getPieChartData(keywordData.position, CONFIG.position)} />
            ...
            <div className='py-3 flex items-center w-full border-b border-dashed border-gray-200'>
                ...
                {
                    keywordData.position && (
                        <Pill color={getColorAndText(keywordData.metaTitle?.length || 0, CONFIG.metaTitle).color} variant='icon' className='ml-auto mr-2 flex gap-2 items-center'>
                            <PieChartUserResultDetails item={getPieChartData(keywordData.metaTitle?.length || 0, CONFIG.metaTitle)} />
                            {getColorAndText(keywordData.metaTitle?.length || 0, CONFIG.metaTitle).text}
                        </Pill>
                    )
                }
            ...
            <div className='py-3 flex items-center w-full border-b border-dashed border-gray-200'>
                ...
                {
                    keywordData.position && (
                        <Pill color={getColorAndText(keywordData.metaDescription?.length || 0, CONFIG.metaDescription).color} variant='icon' className='ml-auto mr-2 flex gap-2 items-center'>
                            <PieChartUserResultDetails item={getPieChartData(keywordData.metaDescription?.length || 0, CONFIG.metaDescription)} />
                            {getColorAndText(keywordData.metaDescription?.length || 0, CONFIG.metaDescription).text}
                        </Pill>
                    )
                }
...
*/
const getPieChartData = (value: number, config: ConfigItem[]): { activePieColor: string, pieColor: string, data: { name: string, value: number }[] } => {
...
*/
const getColorAndText = (value: number, config: ConfigItem[]): { color: PillColor, text: string } => {
    for (let item of config) {
        if (value >= item.min && value <= item.max) {
