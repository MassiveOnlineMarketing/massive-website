...
const KeywordTableHead = () => {
    ...
    return (
        <div className="w-full mb-6 flex items-center border-b border-gray-200 ">
            <p className="mb-2 text-2xl leading-8 font-medium text-gray-800">Keyword Tracker</p>
            <button className='ml-auto' onClick={() => setOpen(true)}>
                <PencilIcon className=" h-4 w-4 text-gray-400" />
            </button>

            <GoogleSearchProjectFormDialog open={open} setOpen={setOpen} googleSearchProject={currentGoogleSearchProject} />
        </div>
    )
}
