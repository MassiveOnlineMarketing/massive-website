...
const RelatedSearchesComponent = ({ keywordData }: { keywordData: KeywordResultWithTagProp }) => {

    const currentProject = useGoogleSearchProjectDetailsStore(state => state.ProjectDetails)
        ...
        <div className='mb-6 min-h-24'>
            <form onSubmit={handleSubmit}>
                <div className='mb-3 flex justify-between items-end'>
                    <p className='text-lg leading-7 font-medium text-gray-800'>Related Searches</p>
                    <Button disabled={isLoading || selectedSearches.length === 0} size='sm' type='submit' variant='appWhite'><PlusIcon className='w-4 h-4' />Add</Button>
                </div>
                {Array.isArray(keywordData.relatedSearches) && keywordData.relatedSearches.length > 0 ? (
                    <>
                        {keywordData.relatedSearches.map((relatedSearch: any) => (
                            <label className='flex items-center gap-[10px]' key={relatedSearch.query}>
                                <input
                                    type='checkbox'
                                ...
                                />
                                <p className="text-base leading-6 font-normal text-gray-800">{relatedSearch.query}</p>
                            </label>
                        ))}
                    </>
                ) : (
                    <p className="text-base leading-6 font-normal text-gray-800">No related searches</p>
                )}
            </form>
        </div>
    )
