...
const PeopleAlsoAsk = ({ keywordData }: { keywordData: KeywordResultWithTagProp }) => {
    return (
        <div>
            <p className='mb-3 text-lg leading-7 font-medium text-gray-800'>People Also Ask</p>
            {Array.isArray(keywordData.peopleAlsoAsk) && keywordData.peopleAlsoAsk.length > 0 ? (
                <>
                    {keywordData.peopleAlsoAsk.map((peopleAlsoSearchedFor: any) => (
                        <div key={peopleAlsoSearchedFor.question} className=' py-[10px] border-t border-gray-200 text-base leading-6 font-normal text-gray-800'>{peopleAlsoSearchedFor.question}</div>
                    ))}
                </>
            ) :
                (
