...
import { GoogleAdsSvg, GoogleSearchConsoleSvg, GreenCheckmarkRound } from '@/assets/icons';
...
const Page = () => {
  ...
  return (
    <>
      <div className='mb-16'>
        <h2 className='font-semibold text-2xl text-gray-800'>Integration & Authorization Settings</h2>
        <p className='max-w-[650px] text-base leading-6 font-normal text-gray-500'>Set the appropriate authorizations to securely access the necessary features and integrations for your workflow</p>
      </div>
      <div className='mb-6'>
...
const GoogleServiceCard: React.FC<GoogleCardProps> = ({ heading, subHeading, Icon, scope, decodedToken, accountDetails, currentlyAvailable }) => {
  // No Google Account is connected
  if (!accountDetails) {
    return (
      <IntegrationCard heading={heading} subHeading='Connect google account before Search' Icon={Icon}>
...
const GoogleAccountCard: React.FC<GoogleCardProps> = ({ heading, subHeading, Icon, scope, accountDetails }) => {
  if (!accountDetails || !accountDetails?.id_token) {
    return (
      <IntegrationCard heading={heading} subHeading={subHeading} Icon={Icon}>
        <Button
