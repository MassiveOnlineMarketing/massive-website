...
const Page = () => {
  ...
  function filterSignupsByDate(signups: NieuwsbriefSignup[] | null, startDate: Date, endDate: Date): NieuwsbriefSignup[] | undefined {
  ...
  const newSignupsTwoWeeksAgo = filterSignupsByDate(result, twoWeeksAgo, lastWeek);
    ...
    <div className='px-6'>
      {result ? (
        <div className="grid grid-cols-4 mt-4 gap-4">
          ...
          <Card title="New Signups this week" titleCompair='Last week' value={newSignupsLastWeek?.length ?? 0} compairValue={newSignupsTwoWeeksAgo?.length} />
        </div>
      ) : (<p>loading...</p>)}
      <div>
        {result ? (
