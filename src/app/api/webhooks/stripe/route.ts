...
export async function POST(request: Request) {
    ...
    if (event.type === 'checkout.session.completed') {
        ...
        if (session.subscription) {
            ...
            const plan = storeSubcsriptionPlans.find(plan => plan.stripePriceId === subscription.items.data[0].price.id)
        ...
        // Else User has made a one-time purchase
            ...
            const plan = storeOneTimeProducts.find(plan => plan.stripePriceId === stripePriceId)
