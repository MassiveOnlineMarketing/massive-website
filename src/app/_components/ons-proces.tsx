...
const OnsProces = () => {
  ...
  const [card2Class, setCard2Class] = useState(
    "translate-x-[-66px] scale-[0.9]",
  );
  const [card3Class, setCard3Class] = useState(
    "translate-x-[-132px] scale-[0.8]",
  );
    ...
    contentRefs.forEach((ref, index) => {
      if (ref.current) {
        ref.current.style.maxHeight =
          clicked === index ? `${ref.current.scrollHeight}px ` : "0";
      }
    });
  ...
  return (
    <div className="overflow-hidden">
      <div
        className={`${container.maxWidthGutter} mt-[120px] border border-red`}
      >
        <div className="grid md:grid-cols-2">
          ...
          <div className="flex flex-col w-full max-w-[500px] my-auto">
            ...
            <div className="flex flex-col gap-2 mb-10">
              <SubHeading level="h2" size="sm" colorScheme="gradient-primary">
                {ONS_PROCES.title.subHeading}
              </SubHeading>
              <Heading level="h3" size="4xl">
                {ONS_PROCES.title.heading}
              </Heading>
                ...
                >
                  <div className="flex gap-2  ">
                    <div className="min-w-[19px] flex justify-center ">
                      <div
                        className={`w-[1.5px] ${clicked === index ? "bg-[#7857FF]" : "bg-gray-300"} `}
                      ></div>
                    </div>
                    <p className=" pb-4 border-b-[1.5px] border-gray-300">
          ...
          <div
            {...handlers}
            className="relative bg-red-500 w-full my-10 h-[478px] "
          >
            <div
              className={cn(
...
const Title = ({
  ...
  return (
    <>
      <div
        className={`flex gap-2 items-center relative  ${index !== 0 ? "pt-4" : ""}`}
      >
