...
export const Banner = ({ containerStyles }: { containerStyles?: string }) => {
  return (
    <div
      className={cn(`relative ${container.maxWidthGutter}`, containerStyles)}
    >
      <div
        className={`relative overflow-hidden  ${constants.glassFill} shadow-custom-lg rounded-[20px] flex flex-row`}
      >
        ...
        <div className=" p-6 md:pt-12 md:pl-8 md:pb-8 max-w-[650px]">
          <Heading level="h3" size="3xl" className="font-semibold">
            {BANNER.heading}
          </Heading>
          ...
          <InternalAnchor
            ...
            size="sm"
            variant="glass"
            className="mt-6 pr-[10px]"
          >
            {BANNER.buttonLabel} <ArrowLongRightIcon className="w-4 h-4" />{" "}
          </InternalAnchor>
        </div>

        <Image
          src={"/home/massive-isometric-image-mini-modal-444x257-XL.jpg"}
          width={447}
          height={295}
          alt="banner image"
          className="lg:block ml-auto mt-auto hidden"
        />
      </div>
...
const BANNER = {
  ...
  paragraaf:
    "Laat ons uw digitale potentieel ontketenen met strategieën die resultaten leveren. Benieuwd naar de mogelijkheden, plan een kosteloos gesprek met een van onze experts.",
