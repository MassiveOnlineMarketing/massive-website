...
const Hero = () => {
  return (
    <div className='relative pt-[120px] md:pt-[70px] md:pb-[70px]'>
      ...
      <div className={`relative md:pb-16 lg:pb-0 grid md:grid-cols-2 ${container.maxWidthGutter}`}>
        <GridBackground className='absolute top-0 left-0 -z-10' />

        <div className='my-auto lg:pb-[120px]'>
          <Title>
            <SubHeading level="h1" size="sm" colorScheme="glass" variant="pill" className='text-primary-500'>{HERO.subheading}</SubHeading>
          ...
          <InternalAnchor href={DEFAULT_MULTISTEP_FORM_ROUTE} size='md' variant='primary' className='mt-6 w-full md:w-fit'>
            {HERO.buttonLabel}
          </InternalAnchor>
        </div>

        <Image loading='eager' src={'/home/massive-hero-header-man-met-laptop-662x755-XL.png'} width={750} height={730} alt='klant die blij zijn website laat zien' className='py-6 md:py-[104px]' />
      </div>
