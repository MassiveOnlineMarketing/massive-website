...
export const Card = ({ title, value, titleCompair, compairValue }: Props) => {
    ...
    if (compairValue) {
        percentageDifference = parseFloat((((value - compairValue) / compairValue) * 100).toFixed(0));
    } else {
    ...
    return (
        <div className="w-full h-full rounded-2xl shadow-sm p-6 bg-white">
            ...
            {compairValue || compairValue === 0 &&
                <div className='mt-4 inline-flex items-center gap-2 text-xs'>
                    <p className={cn(
                        percentageDifference > 0 ? "bg-green-100 text-green-500" : percentageDifference < 0 ? "bg-red-100 text-red-500" : "bg-white text-white",
