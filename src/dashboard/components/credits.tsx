...
import { userTotalKeywordCount } from '@/dashboard/google-search/actions/user-total-keyword-count'
...
const Credits = ({ user }: { user: ExtendedUser | undefined }): JSX.Element => {
    ...
    const [dailySerpSpendPerProject, setDailySerpSpendPerProject] = useState<DailySerpSpendPerProjectProps[] | []>([])
    ...
    return (
        <div className='absolute top-[22px] right-[22px]'>
            <Tooltip>
                ...
                <TooltipContent className='p-5 bg-white z-50 overflow-hidden rounded-lg shadow-lg text-xs text-primary-foreground '
                    side='bottom'
                    align='end'
                >
                    <TooltipArrow
                    ...
                    <div className='relative mb-2 flex flex-col space-y-2 font-medium'>
                        ...
                        {dailySerpSpendPerProject.map((project) => (
                            <div key={project.projectId} className='text-sm leading-5'>
                                ...
                                <p className='text-gray-400'>- {project.keywordCount} Credits</p>
                            </div>
                        ))}
                    <div className='absolute right-0 bottom-1'><PlusIcon className='w-3 h-3 text-gray-500' /></div>
                    </div>
                    <p className='text-gray-800 text-sm pt-3 border-t border-gray-200'><span className='text-gray-500'>Total:</span> - {dailySerpSpendPerProject.reduce((acc, project) => acc + project.keywordCount, 0)} Credits</p>
                </TooltipContent>
            </Tooltip>
        </div>
    )
