...
async function generateSitemap() {
  const categories = await axios(
    `${process.env.PAYLOAD_BACKEND_URL}/api/categories`,
  );
  const categoryPaths = categories.data.docs.map(
    (category) => `/blog/categorie/${category.slug}`,
  );
  ...
  const getPosts = async function (page) {
    const posts = await axios(
      `${process.env.PAYLOAD_BACKEND_URL}/api/blog-posts?limit=10&page=${page}`,
    );
